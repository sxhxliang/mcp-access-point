...
use crate::{
    config::{CLIENT_MESSAGE_ENDPOINT, SERVER_WITH_AUTH},
    sse_event::SseEvent,
    utils,
};
...
impl SseHandler {
    ...
    pub async fn handle_connection(&self, session: &mut Session) -> Result<bool> {
        // Build SSE headers
        ...
        let message_url = self.build_message_url(session, &session_id)?;
        log::debug!("SSE message url: {message_url}");

        // Subscribe to event channel
        let rx = self.tx.subscribe();

        // Create and handle SSE stream
        self.stream_events(session, &session_id, &message_url, rx)
            .await
    }

    /// Builds SSE message URL with optional auth token
    fn build_message_url(&self, session: &mut Session, session_id: &str) -> Result<String> {
    ...
    /// Gets a reference to the event sender for broadcasting events

...
use pingora::{services::Service, upstreams::peer::HttpPeer};
...
pub struct ProxyUpstream {
    pub inner: config::upstream::Upstream,
    lb: SelectionLB,
...
impl TryFrom<config::upstream::Upstream> for ProxyUpstream {
    type Error = Box<Error>;

    /// Creates a new `ProxyLB` instance from an `Upstream` configuration.
    fn try_from(value: config::upstream::Upstream) -> Result<Self> {
        Ok(Self {
            inner: value.clone(),
...
impl ProxyUpstream {
    pub fn new_with_health_check(upstream: config::upstream::Upstream, work_stealing: bool) -> Result<Self> {
    ...
    pub fn start_health_check(&mut self, work_stealing: bool) {
        if let Some(mut service) = self.take_background_service() {
            ...
            runtime.get_handle().spawn(async move {
                service.start_service(watch_rx, 1).await;
                info!("Service exited.");
            });
    ...
    pub fn upstream_host_rewrite(&self, upstream_request: &mut RequestHeader) {
        if self.inner.pass_host == config::upstream::UpstreamPassHost::REWRITE {
    ...
    fn set_timeout(&self, p: &mut HttpPeer) {
        if let Some(config::upstream::Timeout {
            connect,
            read,
            send,
        }) = self.inner.timeout
...
impl TryFrom<config::upstream::Upstream> for SelectionLB {
    type Error = Box<Error>;

    fn try_from(value: config::upstream::Upstream) -> Result<Self> {
        match value.r#type {
            config::upstream::SelectionType::RoundRobin => {
                Ok(SelectionLB::RoundRobin(LB::<RoundRobin>::try_from(value)?))
            }
            config::upstream::SelectionType::Random => {
                Ok(SelectionLB::Random(LB::<Random>::try_from(value)?))
            }
            config::upstream::SelectionType::Fnv => Ok(SelectionLB::Fnv(LB::<FVNHash>::try_from(value)?)),
            config::upstream::SelectionType::Ketama => {
...
impl<BS> TryFrom<config::upstream::Upstream> for LB<BS>
where
    ...
    BS::Iter: BackendIter,
{
    ...
    fn try_from(upstream: config::upstream::Upstream) -> Result<Self> {
...
impl From<config::upstream::HealthCheck> for Box<(dyn HealthCheckTrait + Send + Sync + 'static)> {
    fn from(value: config::upstream::HealthCheck) -> Self {
        match value.active.r#type {
            config::upstream::ActiveCheckType::TCP => {
                let health_check: Box<TcpHealthCheck> = value.into();
                health_check
            }
            config::upstream::ActiveCheckType::HTTP | config::upstream::ActiveCheckType::HTTPS => {
...
impl From<config::upstream::HealthCheck> for Box<TcpHealthCheck> {
    fn from(value: config::upstream::HealthCheck) -> Self {
...
impl From<config::upstream::HealthCheck> for Box<HttpHealthCheck> {
    fn from(value: config::upstream::HealthCheck) -> Self {
        let host = value.active.host.unwrap_or_default();
        let tls = value.active.r#type == config::upstream::ActiveCheckType::HTTPS;

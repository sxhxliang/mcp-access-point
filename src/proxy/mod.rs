    ...
    T: Identifiable,
{
    ...
    /// 构造一个DashMap<String, Arc<T>>的函数，用于重新加载资源，根据提供的资源列表更新内部映射。
    ///
    /// 该函数会删除映射中不在新资源列表中的条目，并插入或更新新资源列表中的条目。
    ///
    /// # 参数
    /// * `resources` - 包含新资源的向量，每个资源都被包装在 `Arc<T>` 中。
    ///
    /// # 行为
    /// 1. 记录每个新资源的插入/更新日志。
    /// 2. 生成新资源的 ID 集合，并保留映射中与新资源 ID 匹配的条目。
    /// 3. 插入或更新映射中的资源。
    fn reload_resource(&self, resources: Vec<Arc<T>>) {
        // 记录每个新资源的插入/更新日志
